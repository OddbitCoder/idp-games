...
void gdp_draw_row_sprite(uint8_t *sprite_row) {
    ...
    if (len > 0) {
        ...
        for (uint8_t i = 1; i < len; i++) {
            ...
            uint8_t dx = ((b & 63) << 1); // dx = len x 2
            dx |= (q >> 2) & (~q >> 1) & 1; // add 1 if previous was NOT transparent and this one is 
            dx -= (q >> 1) & ~q & 1; // subtract 1 if this one is NOT transparent and next one is
            gdp_wait_ready();
