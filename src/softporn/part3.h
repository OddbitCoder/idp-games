...
void seeSomething(enum objects object, const char* message)
{
...
bool objectIsHere(enum objects object)
{
	return state.objectPlace[object] == state.yourPlace;
}

bool objectIsCarried(enum objects object)
{
	return state.objectPlace[object] == youHaveIt;
}

bool objectCanBeTaken(enum objects object)
{
	const enum objects them[] = {
...
bool placeIsPublic(enum places place)
{
	const enum places there[] = {
...
bool youAreIn(enum places place)
{
...
{
	char status[80];
	setScoreText(status, state.score);
	...
	if (!noImAt)
	{
		...
		{
			writeLongMessage(state.yourPlace + 1, true);
			puts("");
		}
	}
	...
	int statusLine = 1;
	{
		...
		for (i = 0; i < 70; i++)
		{
			if (objectIsHere((enum objects)i))
			{
				if (ct)
					strcat(status, ", ");
				ct++;
...
bool verbIsStandalone(enum verbs verb)
{
	const enum verbs verbs[] =
	{
...
bool verbIsSpecial(enum verbs verb)
{
	const enum verbs verbs[] =
	{
...
bool agiParse(char* str)
{
	...
	{
		haveNoVerb = false;
		state.verb = (enum verbs)(results[0] - 300);
	}
	if (results[1] >= 200 && results[1] <= 299)
	{
		haveNoObject = false;
		state.noun = (enum objects)(results[1] - 200);
	}
	if (haveNoObject && results[1] >= 100 && results[1] <= 199)
	{
		haveNoDirection = false;
		state.direction = (enum directions)(results[1] - 100);
	}
...
void readAndParseCommand()
{
	...
	enum places prevPlace = (enum places)-1;
	char score[40];
