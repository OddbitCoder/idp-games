...
const enum directions firstDirection = north;
const enum directions lastDirection = down;
const enum objects firstObject = desk;
const enum objects lastObject = noObject;
const enum places firstPlace = bHallway;
const enum places lastPlace = nowhere;
const enum verbs firstVerb = go;
const enum verbs lastVerb = noVerb;
...
const enum places origObjectPlace[] =
{
	bHallway, //desk
	bBathroom, //washbasin
	bBathroom, //graffiti
...
{
	enum places objectPlace[69];
	bool placeVisited[32];
	enum places path[32][6];
	enum places yourPlace;
